...
NAV = ["Home", "The Model", "Trajectory Signals", "Portfolio", "Account Plan", "Data", "Assumptions"]
...
# ---------- SCENARIO CONTROLS (now at the top of the Data page) --------------
# The controls live in session_state under these keys so the pipeline can read
# them at the top of each run, while the widgets themselves render on the Data
# page. Moving any control reruns the app and every page recomputes.
_CTRL_DEFAULTS = {
    "set_n_accounts": DEFAULT_N_ACCOUNTS,
    "set_n_live":     DEFAULT_N_LIVE,
    "set_winner":     DEFAULT_WINNER_SHARE,
    "set_noise":      DEFAULT_NOISE_SHARE,
    "set_short":      DEFAULT_TERM_PROBS[0],
    "set_smb":        DEFAULT_SEGMENT_PROBS[0],
    "set_mid":        DEFAULT_SEGMENT_PROBS[1],
    "set_seed":       42,
}


def init_settings():
    """Seed default control values once, so the pipeline can read them before the
    Data page renders the widgets."""
...
def read_settings():
    s = st.session_state
    return dict(n_accounts=int(s["set_n_accounts"]), n_live=int(s["set_n_live"]),
                winner_share=float(s["set_winner"]), noise_share=float(s["set_noise"]),
                short_share=float(s["set_short"]), smb=float(s["set_smb"]),
                mid=float(s["set_mid"]), seed=int(s["set_seed"]))


def render_data_controls():
    """The scenario controls, laid out across the top of the Data page. Each is
    bound to a session_state key, so a change reruns the app and rebuilds the
    dataset and every page - including the tables directly below."""
    st.markdown("<div class='ac-eyebrow'>Scenario controls</div>", unsafe_allow_html=True)
    st.caption("This prototype runs on synthetic data. Move a control and every page "
               "recomputes; the tables below update to match.")

    r1 = st.columns(4)
    r1[0].slider("Historical accounts", 100, 800, step=50, key="set_n_accounts",
                 help="Past accounts used to learn the winning pattern.")
    r1[1].slider("Live accounts to score", 10, 80, step=5, key="set_n_live",
                 help="How many current accounts fill the Portfolio.")
    r1[2].slider("Healthy-account share", 0.40, 0.95, step=0.05, key="set_winner",
                 help="Higher means fewer churners. The main dial on the risk mix.")
    r1[3].slider("Noise share", 0.0, 0.30, step=0.02, key="set_noise",
                 help="Accounts built to fool the model (outcome contradicts usage).")

    r2 = st.columns(4)
    r2[0].slider("Share on 12-month deals", 0.0, 1.0, step=0.05, key="set_short")
    r2[1].slider("SMB share", 0.0, 1.0, step=0.05, key="set_smb")
    r2[2].slider("Mid-market share", 0.0, 1.0, step=0.05, key="set_mid")
    r2[3].number_input("Random seed", 0, 9999, step=1, key="set_seed",
                       help="Same seed reproduces the same data.")

    smb = float(st.session_state["set_smb"]); mid = float(st.session_state["set_mid"])
    seg = [smb, mid, max(0.0, 1.0 - smb - mid)]
    tot = sum(seg) or 1.0
    st.caption(f"Segment mix (normalized to sum 1): SMB {seg[0]/tot:.0%}, "
...
def page_model(dataset):
    ...
    with tab1:
        st.write("Pick a contract length and a Trajectory Signal. The green band is the "
        ...
        _, link_col = st.columns([3, 1])
        link_col.button("Signal Library \u2197", key="ts_link",
                        on_click=go, args=("Trajectory Signals",))
