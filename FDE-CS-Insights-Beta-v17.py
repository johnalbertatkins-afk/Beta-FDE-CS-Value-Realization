...
# green risk signaling, hover explanations on columns, a full sortable account
# portfolio, and a single Assumptions page that carries every caveat.
#
# HOW IT IS ORGANIZED (read top to bottom)
# ----------------------------------------
...
# fewer reds. Center points below sit inside the ranges above.
...
NAV = ["Home", "How it works", "Portfolio", "Account Plan", "Data", "Trajectory Signals", "Assumptions"]
...
def inject_css():
    ...
    <style>
      ...
      .ac-hero p  {{ font-size:1.02rem; color:{MUTE}; max-width:none; margin:0; line-height:1.55; }}
...
def render_data_controls():
    ...
    r1[3].slider("Noise share", 0.0, 0.30, value=float(ss["noise_val"]), step=0.02,
                 ...
                 help="Accounts built to fool the agent (outcome contradicts usage).")
...
def page_home(worklist):
    ...
    st.markdown(f"""
      <div class="ac-hero">
        <h1>See which customers are drifting off the adoption and value realization path throughout the customer journey.</h1>
        <p>This agent learns from your account history what a winning account looks like over time. It tracks the Signals behind that path, your product usage and account activity, and how they trend. It then places every live account on that path in real time and turns the gap into a ranked, owner-assigned action plan for forward deployed engineering and customer success teams.</p>
      </div>
    """, unsafe_allow_html=True)
    st.write("")
    ...
    b2.button("See how the agent learns", use_container_width=True,
...
def page_model(dataset):
    ...
    st.header("How the agent learns the adoption and value realization path")

    st.caption("Each account is scored on its Trajectory Signals - the factors that "
               "reveal where it is heading before renewal.")
    st.markdown(
        "<div class='ac-note'>The <b>time frame</b> is the look-back horizon the agent "
...
def _quadrant_fig(df, big=False):
    ...
    for band in ZONE_BASE:
        m = (d["risk_band"] == band).to_numpy()
        if m.any():
            g = d.loc[m, "total_gap"].to_numpy(dtype=float)
            lo, hi = g.min(), g.max()
            norm = (g - lo) / (hi - lo) if hi > lo else np.full_like(g, 0.5)
            within = 0.15 + 0.70 * (1.0 - norm)        # more gap -> lower in the zone
            d.loc[m, "_yj"] = float(ZONE_BASE[band]) + within + jrng.normal(0, 0.015, int(m.sum()))

    xmax = max(float(d["quarters_to_renewal"].max()), 3) + 0.5
    smax = max(float(d["contract_value"].max()), 1)
    d["_size"] = 10 + (d["contract_value"] / smax) * (40 if big else 30)

    customdata = np.stack([
        d["account_id"].to_numpy(),
        d["risk_band"].to_numpy(),
        d["quarters_to_renewal"].to_numpy(),
        d["contract_value"].to_numpy(),
    ], axis=-1)

    fig = pgo.Figure()
    fig.add_hrect(y0=2, y1=3, fillcolor=GREEN, opacity=0.05, line_width=0, layer="below")
    fig.add_hrect(y0=1, y1=2, fillcolor=AMBER, opacity=0.05, line_width=0, layer="below")
    fig.add_hrect(y0=0, y1=1, fillcolor=RED,   opacity=0.06, line_width=0, layer="below")
    fig.add_vline(x=SOON_MAX_QTRS + 0.5, line=dict(color=INK, width=1, dash="dash"), opacity=0.25)

    fig.add_trace(pgo.Scatter(
        x=d["_xj"], y=d["_yj"], mode="markers",
        marker=dict(size=d["_size"], color=d["risk_band"].map(RISK_COLOR).tolist(),
                    line=dict(color="white", width=1.2), opacity=0.85),
        customdata=customdata,
        hovertemplate=("<b>%{customdata[0]}</b><br>%{customdata[1]}<br>"
                       "Qtrs to renewal: %{customdata[2]}<br>"
                       "Contract value: %{customdata[3]:,}<br>"
                       "<span style='color:#888'>click to open plan</span>"
                       "<extra></extra>"),
        showlegend=False,
    ))

    fig.add_annotation(x=0, y=0.1, text="Act now", showarrow=False, xanchor="left",
                       yanchor="bottom", font=dict(color=RED, size=11, family="monospace"))
    fig.update_xaxes(range=[-0.5, xmax], zeroline=False, showgrid=True,
                     gridcolor="rgba(14,34,51,0.08)",
                     title_text="Time to renewal:  sooner \u2190          \u2192 later")
    fig.update_yaxes(range=[0, 3], zeroline=False, showgrid=False,
                     tickvals=[0.5, 1.5, 2.5],
                     ticktext=["At risk", "Needs attention", "On track"])
    fig.update_layout(height=440 if big else 340, font=dict(color=INK),
                      margin=dict(l=10, r=10, t=10, b=46),
                      plot_bgcolor="white", paper_bgcolor="rgba(0,0,0,0)")
    return fig


def _render_quadrant(df, big, key):
    """Render the interactive chart and open the account plan when a bubble is clicked."""
    fig = _quadrant_fig(df, big=big)
    ...
    if pts:
        ...
        if cd:
            ...
            acct = df.reset_index(drop=True).iloc[p["point_index"]]["account_id"]
        if acct:
            st.session_state.pop(key, None)   # clear so it doesn't re-fire on return
            go("Account Plan", acct)
            st.rerun()


def _portfolio_table(df_sorted, key):
    """Display list of accounts. The account name is a link that opens its plan."""
    show = df_sorted.reset_index(drop=True).copy()

    st.caption("Click on account to open account's plan and actions.")

    cols = [
        ("Account",         "Account identifier"),
        ("Risk",            "Overall renewal risk"),
        ("Segment",         "Customer size tier"),
        ("Term",            "Contract length"),
        ("Now Q",           "Current quarter in term"),
        ("Qtrs to renewal", "Quarters until renewal"),
        ("Off-target",      "Signals behind target"),
        ("Main gaps",       "Largest off-target signals"),
        ("Contract value",  "Credits over full term"),
        ("Priority",        "Higher means act sooner"),
        ("Actual (hidden)", "Real result, the agent never sees it"),
    ]
    th = "".join(
        f"<th title='{help}' style='text-align:left;padding:7px 10px;"
        f"border-bottom:1px solid {LINE};color:{MUTE};font-weight:600;"
        f"font-size:0.82rem;white-space:nowrap;'>{label}</th>"
        for label, help in cols
    )

    rows_html = []
    for _, r in show.iterrows():
        aid = str(r["account_id"])
        link = (f"<a href='?account={quote(aid)}' target='_self' "
                f"style='color:#1A73E8;text-decoration:underline;font-weight:600;'>{aid}</a>")
        cells = [
            link,
            RISK_BADGE.get(r["risk_band"], str(r["risk_band"])),
            str(r["segment"]),
            str(r["term_label"]),
            f"{int(r['current_quarter'])}",
            f"{int(r['quarters_to_renewal'])}",
            f"{int(r['off_target_count'])}",
            str(r["top_drivers"]),
            f"{int(r['contract_value']):,}",
            f"{r['priority_score']:.2f}",
            str(r["true_outcome_hidden"]),
        ]
        tds = "".join(
            f"<td style='padding:7px 10px;border-bottom:1px solid {LINE};"
            f"font-size:0.86rem;white-space:nowrap;'>{c}</td>"
            for c in cells
        )
    ...
    st.markdown(
        ...
        f"<thead><tr>{th}</tr></thead><tbody>{''.join(rows_html)}</tbody></table></div>",
        unsafe_allow_html=True)

    st.caption("The last column is the true outcome, hidden from the agent and shown "
...
def page_portfolio(worklist):
    ...
    with left:
        _render_quadrant(df, big=False, key="pf_chart")
    with right:
...
def _portfolio_focus_view(worklist, focus):
    ...
    _render_quadrant(sub, big=True, key="pf_chart_focus")
...
def _signal_rows_html(tier):
    ...
    for k in SIGNALS:
        ...
        rows.append(
            ...
            f'<span style="color:{MUTE};font-weight:500;">: {definition}</span></div>'
            f'<div style="color:#33424C;font-size:0.9rem;margin-top:2px;"><i>How the agent reads it:</i> {calc}</div>'
            f'{note_html}</div>')
    return "".join(rows)
...
def page_trajectory_library():
    ...
    st.write("The signals the agent weighs to see where an account is heading - toward "
    ...
    subs = [
        ...
        ("Governing principle",
         "No single signal is treated as true. The agent learns which signals, and in what combination, actually "
...
def page_data(dataset):
    ...
    for name in ["accounts", "account_quarter", "account_quarter_feature", "event_log"]:
        df = dataset[name]
        st.markdown(f"**{name}**<span style='color:{MUTE}'>: {labels[name]} "
...
def page_assumptions():
    ...
    st.write("An account's fate is labeled from where its end-of-term consumption lands "
    ...
    st.write("The time frame is the window the agent factors, not a signed contract "
             "term. This is deliberate. Enterprise pricing is moving toward committed "
             "consumption over a fixed period, so the contract itself keeps getting "
             "shorter and less predictable, while the agent only needs a window to work "
             "from. The showcase offers three time frames (12, 24, and 36 months) to show "
             "how the same method holds across spans.")
    st.write("Each time frame is learned separately, measured in quarters within it, "
             ...
             "fraction of the time frame for each, which in real time arrives sooner for the "
             f"shorter ones. The mix defaults short ({int(DEFAULT_TERM_PROBS[0]*100)}% "
    ...
    st.write(f"The bands are calibrated assuming a company brings about {HISTORY_YEARS} "
             ...
             "so the agent is weakest on day one and compounds from there. Companies with "
    ...
    st.write("About one in eight accounts is deliberate \"noise\": strong usage that still "
             ...
             "costly to switch). They are included so we can later test how often the agent "
    ...
    st.markdown(
        ...
        "- Only the first time frame is modeled, so survivorship is handled lightly.\n"
        "- Forward forecasting (look-forward) is out of scope here; this is a look-back, at-risk-detection model.\n"
        "- The play library (the specific recommended actions) is the one piece only your organization can supply.")
...
def main():
    ...
    # In-text links (e.g. "See Signal Library Definitions" on the How it works page)
    ...
    # An account-name link on the Portfolio table opens that account's plan.
    acct_qp = st.query_params.get("account")
    if acct_qp:
        st.session_state.selected_account = acct_qp
        st.session_state.page = "Account Plan"
        st.query_params.clear()
