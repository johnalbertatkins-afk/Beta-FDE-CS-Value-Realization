...
def read_settings():
    ...
    return dict(n_accounts=int(s["n_accounts_val"]), n_live=int(s["n_live_val"]),
                ...
                short_share=float(s["short_val"]), smb=float(s["smb_val"]),
...
def render_data_controls():
    ...
    r1 = st.columns(4)
    r1[0].slider("Historical accounts", 100, 800, value=int(ss["n_accounts_val"]), step=50,
    ...
    r2 = st.columns(4)
    r2[0].slider("Share on 12-month deals", 0.0, 1.0, value=float(ss["short_val"]), step=0.05,
                 key="short_w", on_change=_sync, args=("short_w", "short_val"))
    r2[1].slider("SMB share", 0.0, 1.0, value=float(ss["smb_val"]), step=0.05,
                 key="smb_w", on_change=_sync, args=("smb_w", "smb_val"))
    r2[2].slider("Mid-market share", 0.0, 1.0, value=float(ss["mid_val"]), step=0.05,
                 key="mid_w", on_change=_sync, args=("mid_w", "mid_val"))
    r2[3].number_input("Random seed", 0, 9999, value=int(ss["seed_val"]), step=1,
